...
const INCOMING_MESSAGE_TEXT = 'ou comment je me suis retrouvé·e bloqué·e dans la tête de deux femmes au bord de l’explosion'
...
const keys: { main: string; sub: string }[][] = [
  [{ main: 'salut', sub: '' }, { main: '2', sub: 'ABC' }, { main: '3', sub: 'DEF' }],
  [{ main: '4', sub: 'GHI' }, { main: 'ça va', sub: '' }, { main: '6', sub: 'MNO' }],
  [{ main: '7', sub: 'PQRS' }, { main: '8', sub: 'TUV' }, { main: 'et toi ?', sub: '' }],
...
function Bubble({ text, bubbleId }: { text: string; bubbleId: string }) {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      ...
      { scale: 1, opacity: 1, y: 0, duration: BUBBLE_APPEAR_DURATION, ease: 'back.out(1.7)' }
    )
  }, [])
...
function SentChatBubble({ text, itemId, href }: { text: string; itemId: string; href?: string }) {
  const ref = useRef<HTMLDivElement>(null)
...
export default function Dialpad() {
  ...
  }, [showChat, showReply, showSecondReply, showThirdReply, showFourthReply, messages.length])

  useEffect(() => {
    if (messages.length !== 3 || animationRanRef.current) return

    let frame = 0
    const timeout = window.setTimeout(() => {
      frame = requestAnimationFrame(() => {
        const bubbleEls = Array.from(bubblesRowRef.current?.querySelectorAll('.bubble-wrapper') ?? []) as HTMLDivElement[]
        const bubbleInnerEls = bubbleEls.map((el) => el.querySelector('.bubble-sent')).filter(Boolean) as HTMLSpanElement[]
        const targetEls = Array.from(chatTargetsRef.current?.querySelectorAll('.bubble-target') ?? []) as HTMLSpanElement[]

        if (bubbleEls.length !== 3 || bubbleInnerEls.length !== 3 || targetEls.length !== 3) return

        animationRanRef.current = true
        setIsAnimatingToChat(true)

        const startRects = bubbleInnerEls.map((el) => el.getBoundingClientRect())
        ...
        tl.to(bottomRef.current, { opacity: 0, duration: 0.55, ease: 'power2.out' }, 0)
  ...
  return (
    <div ref={chatAreaRef} className="relative h-full flex flex-col overflow-hidden">
      <div
        ref={chatTargetsRef}
        className={`pointer-events-none absolute inset-0 px-4 ${showChat ? 'opacity-100' : 'opacity-0'}`}
        aria-hidden={!showChat}
      >
        <div ref={chatStackRef} className="h-full flex flex-col justify-center gap-3">
          {messages.map((msg, i) => (
            <div key={i} data-chat-item-id={`sent-${i}`} className="flex justify-end">
              <span className="bubble-sent bubble-target">{msg}</span>
            </div>
          ))}
          {showReply && <ReplyBubble itemId="reply" text={INCOMING_MESSAGE_TEXT} />}
          {showSecondReply && <SentChatBubble itemId="reply-sent" text={SECOND_REPLY_TEXT} />}
          {showThirdReply && <SentChatBubble itemId="reply-link" text={THIRD_REPLY_TEXT} href={THIRD_REPLY_TEXT} />}
          {showFourthReply && <ReplyBubble itemId="reply-final" text={FOURTH_REPLY_TEXT} />}
        </div>
      </div>
      ...
      {showBottom && (
        <div ref={bottomRef} className="flex flex-col items-center gap-4 px-4 pb-8 pt-4">
          <div className="w-full max-w-xs flex items-center justify-between border-b border-zinc-200 pb-3">
            <span className="text-3xl tracking-widest font-light text-zinc-800 flex-1">{number}</span>
            {number && (
              <button onClick={backspace} className="text-zinc-400 hover:text-zinc-700 transition-colors px-2" aria-label="Effacer">
                ⌫
              </button>
            )}
          </div>
              ...
              <div key={i} className="flex gap-3 justify-center">
                {row.map((key) => (
                  <button
                    ...
                    disabled={!messageKeys.has(key.main) || done}
                    className="w-20 h-20 rounded-full flex flex-col items-center justify-center transition-colors disabled:opacity-40 disabled:cursor-not-allowed enabled:active:bg-zinc-200"
                    style={{ background: 'rgba(120,120,128,0.12)' }}
                  >
                    <span
                      className="font-light leading-none text-zinc-900"
                      style={{ fontFamily: '-apple-system, sans-serif', fontSize: messageKeys.has(key.main) ? '13px' : '28px' }}
                    >
                      {key.main}
                    </span>
                    {key.sub && (
                      <span className="text-[10px] tracking-widest font-medium text-zinc-900 mt-0.5">{key.sub}</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          <button disabled className="w-20 h-20 rounded-full bg-green-500 opacity-30 cursor-not-allowed text-white text-2xl" aria-label="Appeler">
            📞
          </button>
        </div>
      )}
